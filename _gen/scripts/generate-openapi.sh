...
function resolve_refs {
  input="${1:-$(</dev/stdin)}";
  swagger-cli bundle -r <(echo "$input")
}
...
function merge_allOf {
  ...
  jqcmd+=' | $root | keys | map($root | paths(.)) | map(if (. | index("allOf")) then . else null end) | reduce .[] as $item ([]; if $item != null then . + [$item + [($item | rindex("allOf"))]] end) | map(.[:(.[-1])]) | unique as $nodes'
